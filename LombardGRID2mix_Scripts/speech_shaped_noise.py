"""
File originates from the pyAcoustics repository (https://github.com/timmahrt/pyAcoustics/tree/main).
Some functions have been slightly adapted to work with the given package versions in the 
Lombard2mix_Scripts repository.
...
def _write_wav(fname, fs, x, normalize=False):
    ...
    if np.issubdtype(x.dtype, float) and normalize:
        scaled = x / np.max(np.abs(x)) * (2**15 - 1)
    elif np.issubdtype(x.dtype, float):
        scaled = x * (2**15 - 1)
    else:
        scaled = x
...
def _getFramerate(wavFN):
    samplerate, data = wavfile.read(wavFN)
